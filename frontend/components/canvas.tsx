...
export default function Canvas() {
  ...
  const handleMouseDown = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    if (event.button !== 0 || isTwoFingerPanning) return;
  ...
  const handleGlobalMouseMove = useCallback((event: MouseEvent) => {
    ...
    const dx: number = event.clientX - lastMousePos.x;
    const dy: number = event.clientY - lastMousePos.y;
    setPanOffset(prev => ({ x: prev.x + dx, y: prev.y + dy }));
    setLastMousePos({ x: event.clientX, y: event.clientY });
  }, [isMousePanning, lastMousePos]);
  ...
  }, [isMousePanning, handleGlobalMouseMove, handleGlobalMouseUp]);
  ...
  return (
    <div
      ref={viewportRef}
      className="w-full h-full bg-gray-50 rounded-md shadow-lg overflow-hidden relative select-none touch-none" // touch-none is key
      style={{ cursor: 'grab' }}
      onWheel={handleWheel}
