...
// HasParticipantPermission returns a set of RoleType values that are granted participant-level permissions.
// The set includes roles such as Host, Screenshare, and Participant.
func HasParticipantPermission() set.Set[RoleType] {
	return set.New(RoleTypeHost, RoleTypeScreenshare, RoleTypeParticipant)
}

// HasScreensharePermission returns a set of RoleType values that are permitted to use the screenshare feature.
// The set typically includes roles such as Host and Screenshare, granting them the necessary permissions.
